...
void Fallback(int from_fd, int to_fd, loff_t begin, loff_t end, boost::progress_display &progress) {
...
void CopyRange(int from_fd, int to_fd, loff_t begin, loff_t end) {
  boost::progress_display progress(end - begin, std::cerr, std::string("Bytes [") + boost::lexical_cast<std::string>(begin) + "," + boost::lexical_cast<std::string>(end) + ")\n");
  Fallback(from_fd, to_fd, begin, end, progress);
  /*
  size_t progress_stop = std::max<size_t>((end - begin) / 100, 4096);
  while (begin < end) {
    ssize_t ret = splice(from_fd, &begin, to_fd, NULL, std::min<size_t>(end - begin, progress_stop), 0);
    if (ret < 0) {
      if (errno == EINVAL) {
        Fallback(from_fd, to_fd, begin, end, progress);
        return;
