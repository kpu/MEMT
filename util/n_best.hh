...
/* A note on threadsafety: const methods are safe for parallel access iff
 * HeapDropper is not used.  
 ...
 * Disclaimers:
 * const methods are safe for parallel access iff HeapDropper is not used.
 *
 * Duplicate detection interacts interestingly with falling off the n-best
 * list.  Specifically, the score of a hypothesis may be below the beam
 * threshold i.e. MayMakeIt returns false.  The hypothesis will still be merged
 * if there is a duplicate higher scoring hypothesis already in the beam.
 * 
